...
namespace TsdClient
{
    internal sealed class TsdMetrics : IMetrics
    {
        ...
        public TsdMetrics(IEnumerable<ISink> sinks, string service, string cluster, string host)
        {
            this.Log = LogManager.GetLogger(typeof(TsdMetrics));
            Sinks = sinks;
        ...
        private ITimer InternalCreateTimer(string name, Int64 elapsed)
        {
        ...
        public void StartTimer(string name)
        {
            ...
            if (!Timers.TryGetValue(name, out timer))
                ...
                Log.Warn(m => m("Timer {0} is already started.", timer.Name));
        ...
        public void SetTimer(string name, long duration, TimeUnit unit)
        {
            if (!AssertIsOpen())
                return;
            (InternalCreateTimer(name, duration) as TsdTimer).Unit = unit;
        }
        ...
        private bool SelectAlways<T>(IQuantity<T> v)
        {
            return true;
        }
